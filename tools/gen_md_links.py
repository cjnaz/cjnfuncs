...
# for match in p.finditer(all):
#     print (match.group(1).split('\n')[0])

shutil.copy2("README.head", README)
...
for match in p.finditer(all):
    ...
    # print (funcline)
    xx = funcline.replace("Class ", "").split(maxsplit=1)
    # print (xx)
    link_name = xx[0]

    # link_name = xx.split("(")[0].replace("Class", "").strip()
    link = funcline.replace(" ", "-").lower()
    # link = xx[1].replace(" ", "-").lower()
    # link = xx.lower().strip()
    deleted = ":()\n,.=\"\'"
    for char in deleted:
...
with pathlib.Path(README).open('a') as outfile:
    outfile.write(links)
...
with pathlib.Path("README.md", 'w').open() as outfile:
    for line in infile: #.readline():
        if line.startswith('##'):
            if line.startswith("###"):
                continue
            # print ("\n", line.replace("\n", ""))
            xx = line.split(maxsplit=1)[1]
            link_name = xx.split("(")[0].replace("Class", "").strip()
            link = xx.replace(" ", "-").lower()
            # link = xx.lower().strip()
            deleted = ":()\n,.=\"\'"
            for char in deleted:
                link = link.replace(char, "")
